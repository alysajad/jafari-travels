...
let content = fs.readFileSync('src/data/site.ts', 'utf8');

const newPackages = `export const packages = [
  {
...
content = content.replace(/export const packages = \[\s*\{[\s\S]*?\}\s*\];/m, newPackages);
fs.writeFileSync('src/data/site.ts', content);
